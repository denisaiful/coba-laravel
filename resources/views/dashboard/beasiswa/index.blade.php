...
<a href="/dashboard/beasiswa/create" class="shadow p-2 rounded btn btn-success mb-3">Tambah Penerima Beasiswa</a>
        ...
        <table id="example" class="ui celled table" >
          ...
          @foreach($beasiswas as $beasiswa)
            <tr>
              ...
              <td>@if ($beasiswa->faculty)
                {{ $beasiswa->faculty->name}}
                @endif
              </td>
              <td>{{ $beasiswa->status }}</td>
              <td>
              <a href="/dashboard/beasiswa/{{ $beasiswa->id }}/edit" class="badge bg-warning"> <span data-feather="edit"></span></a>
              
              <form Action="/dashboard/beasiswa/{{ $beasiswa->id }}" method="post" class="d-inline">
              @method('delete')
              @csrf
